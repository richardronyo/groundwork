...
Usage:
    python3 file_dependencies.py <files.json> --repo <path/to/repo>
    python3 file_dependencies.py flask_files.json --repo ./flask --lines 30
    python3 file_dependencies.py files.json --repo /path/to/repo --from-db

Requirements:
    pip install kuzu psycopg python-dotenv
"""
...
# --- For --from-db mode ---
import psycopg
from kb.relationaldb.initialize_db import DB_CONFIG
...
# ── New extraction from PostgreSQL database ──────────────────────────────────

def extract_dependencies_from_db(
    files: list[dict],
    repo_root: Path,
    repo_name: str
) -> list[tuple[str, str]]:
    """
    Reads import_names from PostgreSQL instead of extracting from files.
    """
    disk_base = repo_root.resolve().parent
    lookup = build_lookup(files)
    edges = []
    ...
    with conn.cursor() as cur:
        # Fetch all files' import_names for this repo in one query
        cur.execute("""
            SELECT file_path, import_names
            FROM files
    ...
    # Build a dict: file_path -> list of imported names
    # psycopg3 (unlike psycopg2) auto-adapts JSONB columns into native Python
    # objects on fetch, so `import_json` here is normally already a list —
    # only call json.loads() if it actually came back as text.
    db_imports = {}
    for path, import_json in rows:
        if import_json is None:
            db_imports[path] = []
        elif isinstance(import_json, str):
            db_imports[path] = json.loads(import_json)
        else:
            db_imports[path] = import_json

    for file_meta in files:
        rel = file_meta["relative"]
        language = file_meta.get("language", "")
        raw_imports = db_imports.get(rel, [])
...
def main():
    ...
    parser.add_argument("--from-db", action="store_true",
                        help="Read imports from PostgreSQL 'import_names' column instead of extracting from files")
    ...
    if args.from_db:
        print("  Using imports from PostgreSQL (import_names column).")
