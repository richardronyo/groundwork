...
Populates the embedded Kùzu graph with File / Directory nodes and
CONTAINS / SAME_DIR edges from a list of file metadata.
...
Programmatic usage:
    ...
    ingester = GraphIngester(repo_name="flask", db_path="./kuzu_db")
    ingester.ingest(files)          # files is a list of dicts
    ingester.close()
...
# ── Code language filter (used by CLI) ────────────────────────────────────────
...
class GraphIngester:
    """
    Ingests file/directory hierarchy into Kùzu.
    """
    ...
    def close(self):
        """Close connection (no‑op for embedded)."""
        pass
    ...
    def ingest(self, files: list[dict]):
        """
        Ingest the given list of file metadata into Kùzu.
        Each dict must have at least:
            'relative': str (e.g., "repo/src/file.py"),
            'name': str,
            'extension': str,
            'language': str,
...
def main():
    ...
    # Filter to only code files (same as before)
